	...
	<div class="container allratings">
		<div class="row">
			...
			<div class="col-6">
				...
				$query = "SELECT movies.movie_name, movie_reviews.ratings, movie_reviews.user_id, movie_reviews.movie_id FROM movie_reviews JOIN movies ON movie_reviews.movie_id = movies.movie_id WHERE movie_reviews.user_id='$userid'";
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					if ($_GET["user"] == $_SESSION["username"]) {
						...
						echo htmlspecialchars('/movies1/yourreview.php?user=' . $user . '');
						echo '">';
						echo "
				...
				if (isset($_POST["delrecord"])) {
					...
					$query = "DELETE FROM movie_reviews WHERE movie_reviews.movie_id='$movieid' AND movie_reviews.user_id='$userid'";
				...
				if (isset($_POST["rate"])) {
					...
					$query = "UPDATE movie_reviews SET movie_reviews.ratings = '$rating' WHERE movie_id = '$movieid' AND user_id = '$userid'";
					$query_run = mysqli_query($con, $query);
					if ($query_run) {
