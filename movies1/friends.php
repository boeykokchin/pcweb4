				...
				<div class="col-6">
					...
					<div class="searchresult">
						<?php
							...
							$query = "SELECT userinfo.username, user_relation.follower_id, user_relation.followee_id FROM user_relation LEFT JOIN userinfo ON userinfo.user_id = user_relation.followee_id WHERE user_relation.follower_id='$followerid'";
							...
							if(isset($_POST["delrecord"])){
								...
								$query = "DELETE FROM user_relation WHERE user_relation.followee_id='$followeeid'";
								$query_run = mysqli_query($con, $query);
								if($query_run){
						...
						<div class="results">
							<?php
								if(isset($_POST["search"])){
									...
									$query = "SELECT userinfo.username FROM userinfo WHERE userinfo.username LIKE '%".$search."%'";
									$result = mysqli_query($con, $query);
									while($row = mysqli_fetch_array($result)){
										$name = $row['username'];
